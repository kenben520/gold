...
public class MinuteChartView extends View implements GestureDetector.OnGestureListener {
    ...
    private void init() {
        ...
        mAvgPaint.setStrokeWidth(dp2px(3f));
    ...
    private void drawView2(Canvas canvas){
        int mEndIndex = mPoints.size();
        if (mPoints.size() > 0) {
            IMinuteLine lastPoint = mPoints.get(0);
            Path path = new Path();
            float lastX = getX(0);
            path.moveTo(lastX,getHeight());
        //    path2.moveTo(0,  getY(lastPoint.getPrice()));
            for (int i = mBeginIndex + 1; i < mEndIndex; i++) {
                ...
                lastPoint = curPoint;
                lastX=curX;
//                path.moveTo(curX, getY(lastPoint.getPrice()));
                path.lineTo(lastX,getY(curPoint.getAvgPrice()));
//                path.lineTo(k, getHeight());
//                path.close();
            }
            path.lineTo(lastX,getHeight());
//            path.close();
            Paint paint = new Paint();
            paint.setAntiAlias(true); // 去锯齿
//                paint.setColor(Color.pack(223,236,254));
