...
public class BOLLDraw implements IChartDraw<IBOLL> {
    ...
    public void drawTranslated(@Nullable IBOLL lastPoint, @NonNull IBOLL curPoint, float lastX, float curX, @NonNull Canvas canvas, @NonNull BaseKChartView view, int position) {
        view.drawMainLine(canvas, mUpPaint, lastX, lastPoint.getMb(), curX, curPoint.getMb());
        view.drawMainLine(canvas, mMbPaint, lastX, lastPoint.getUp(), curX, curPoint.getUp());
    ...
    public void drawText(@NonNull Canvas canvas, @NonNull BaseKChartView view, int position, float x, float y) {
        ...
        text = "MID:" + view.formatValue(point.getMb()) + "  ";
        canvas.drawText(text, x, y, mUpPaint);
        x += mUpPaint.measureText(text);
        text = "UPPER:" + view.formatValue(point.getUp()) + "  ";
        canvas.drawText(text, x, y, mMbPaint);

        y += mTextSize;
        text = "LOWER:" + view.formatValue(point.getDn());
        canvas.drawText(text, left, y, mDnPaint);
    }
