...
public class TabView extends RelativeLayout {
    ...
    private void initView(Context context) {
        ...
        tabview = scrollTabView.findViewById(R.id.tabview_id);
        ...
        if (mTabViewGravity == Gravity.BOTTOM) {
            ...
            fragmentContainerParams.addRule(RelativeLayout.ABOVE, R.id.scroll_tabview);
            ...
            fragmentContainerParams.addRule(RelativeLayout.RIGHT_OF, R.id.scroll_tabview);
        } else if (mTabViewGravity == Gravity.TOP) {
            tabviewParams = new RelativeLayout.LayoutParams(RMP, mTabViewHeight);
            tabview.setOrientation(LinearLayout.HORIZONTAL);
            tabviewParams.addRule(RelativeLayout.ALIGN_PARENT_TOP);
            fragmentContainerParams.addRule(RelativeLayout.BELOW, R.id.scroll_tabview);
        } else if (mTabViewGravity == Gravity.RIGHT) {
            tabviewParams = new RelativeLayout.LayoutParams(mTabViewHeight, RMP);
            tabview.setOrientation(LinearLayout.VERTICAL);
            tabviewParams.addRule(RelativeLayout.ALIGN_PARENT_RIGHT);

            fragmentContainerParams.addRule(RelativeLayout.LEFT_OF, R.id.scroll_tabview);
        }
        scrollTabView.setLayoutParams(tabviewParams);
        scrollTabView.setBackgroundColor(mTabViewBackgroundColor);

        mFragmentContainer.setLayoutParams(fragmentContainerParams);
        this.addView(scrollTabView);
    ...
    private void initTabChildView() {
        ...
        for (int i = 0; i < mTabViewChildList.size(); i++) {
            final TabViewChild t = mTabViewChildList.get(i);
            View tabChild = getTabItemView(0);
            tabview.addView(tabChild);
            final TextView textView = (TextView) tabChild.findViewById(R.id.textview);
            final View vl_1 = tabChild.findViewById(R.id.vl_1);
            final View hl_1 = tabChild.findViewById(R.id.hl_1);
            final View layout_item = tabChild.findViewById(R.id.layout_item);
            ...
            if (mTabViewDefaultPosition >= mTabViewChildList.size()) {
                if (i == 0) {
                    ...
                    vl_1.setVisibility(VISIBLE);
                    hl_1.setVisibility(GONE);
                    layout_item.setBackgroundColor(getContext().getResources().getColor(R.color.whiteTwo));
                }
            } else {
...
//                        listener.onTabChildClick(currentPosition,imageview,textview);
    ...
    public View getTabItemView(int position) {
        View view = LayoutInflater.from(getContext()).inflate(R.layout.signal_set_item, null);
        return view;
    }

    public View getScrollTabView() {
        View view = LayoutInflater.from(getContext()).inflate(R.layout.signal_params_set_tab_view, null);
        return view;
    }
    ...
    private void reSetAll() {
        for (int i = 0; i < tabview.getChildCount(); i++) {
            ...
            for (int j = 0; j < tabChild.getChildCount(); j++) {
                TextView textView = (TextView) tabChild.findViewById(R.id.textview);
                View vl_1 = tabChild.findViewById(R.id.vl_1);
                View hl_1 = tabChild.findViewById(R.id.hl_1);
                View layout_item = tabChild.findViewById(R.id.layout_item);
                layout_item.setBackgroundColor(getContext().getResources().getColor(R.color.white));
                textView.setSelected(false);
                vl_1.setVisibility(GONE);
            }
        }
    }
