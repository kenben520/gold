...
import static com.lingxi.preciousmetal.ui.activity.QuestionActivity.nickName;
import static com.lingxi.preciousmetal.ui.fragment.analyzeplate.PolyvChatFragment.questionMessagesHmp;

//import com.bumptech.glide.load.resource.drawable.GlideDrawable;

public class PolyvQuestionAdapter extends BaseAdapter implements OnClickListener {
    public static List<PolyvChatMessage> messages;
...
//    private List<PolyvChatMessage> allMessages;
    ...
    public PolyvQuestionAdapter(Context context, List<PolyvChatMessage> messages, ListView lv_chat) {
...
//        this.teacherMessages = filter(messages);
    ...
    public void clear() {
//        questionMessagesHmp.get(nickName).clear();
        messages.clear();
//        allMessages.clear();
//        teacherMessages.clear();
    ...
    public void add(PolyvChatMessage message, boolean needAddGlobalQuestionList) {
        if (onlyTeacher) {
            final PolyvChatMessage.User user = message.getUser();
            if (isTeacher(user)) {
        ...
        if (needAddGlobalQuestionList) {
            questionMessagesHmp.get(nickName).add(message);
        }
...
//        teacherMessages.addAll(0, filter(lists));
        ...
        questionMessagesHmp.get(nickName).addAll(0, lists);
        notifyDataSetChanged();
    }
...
//    }
    ...
    public PolyvChatMessage removeQuestionMsg(int position) {
        PolyvChatMessage chatMessage = questionMessagesHmp.get(nickName).remove(position);
        notifyDataSetChanged();
        return chatMessage;
    }
    ...
    private View getChatView(int position) {
        switch (getItemViewType(position)) {
            ...
            case PolyvChatMessage.CHATTYPE_RECEIVE_QUESTION:
                return inflater.inflate(R.layout.polyv_listivew_question_receive, null);
            ...
            case PolyvChatMessage.CHATTYPE_SEND:
                return inflater.inflate(R.layout.polyv_listview_question_send, null);
        }
        return null;
    }
...
//        }
        ...
        if (message.getChatType() != PolyvChatMessage.CHATTYPE_RECEIVE_NOTICE) {
            if (message.getChatType() == PolyvChatMessage.CHATTYPE_RECEIVE_QUESTION) {//私聊和公聊返回数据不是在同一个字段 私聊这里是老师的回复内容 没有时间
                textImageLoader.displayTextImage(message.getContent(), tv_msg);
        ...
        if (view != null) {
            if (isSendSuccess) {
                if (isReSend && messages.size() > 1) {
                    ...
                    questionMessagesHmp.get(nickName).add(removeQuestionMsg(position));
